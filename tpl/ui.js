...
window.UI = (function () {
  ...
  const statusTag = s => {
    const m = TPL.STATUS[s] || { c: '#9a9a9a' };
    return `<span class="tag" style="--tc:${m.c};background:${m.c};color:${s === 'Delivered' ? '#fff' : '#000'};border-color:transparent">${esc(s)}</span>`;
  };

  /* read-only vs editable — the organising idea of Dash 3PL */
  const mode = (kind, why) =>
    `<div class="modebar ${kind}"><span class="mb-t">${kind === 'ro' ? 'Read only' : 'You control this'}</span><span class="mb-w">${why}</span></div>`;
  ...
  const note = (title, body, color) =>
    `<div class="note" style="--nc:${color || TPL.PAL.lemon}"><b>${esc(title)}</b> ${body}</div>`;
  ...
  return { esc, page, btn, tag, statusTag, mode, kpi, panel, table, bar, toggle, field, input, select, radio, tabs,
