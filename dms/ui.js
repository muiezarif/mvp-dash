...
window.UI = (function () {
  ...
  const note = (title, body, color) =>
    `<div class="note" style="--nc:${color || DMS.PAL.lemon}"><b>${esc(title)}</b> ${body}</div>`;
  ...
  return { esc, page, btn, tag, statusTag, kpi, panel, table, bar, toggle, field, input, select, radio, tabs,
